...
public class SolutionSet implements Serializable {
	...
	// add by Miao
	// print solutions, including variables and objectives
	public void printSolutions(PrintWriter pw) {
		...
		if (size() > 0) {
			...
			for (Solution aSolutionsList_ : solutionsList_) {
				// print variables
				for (int j = 0; j < numberOfVariables; j++)
					pw.print(aSolutionsList_.getDecisionVariables()[j]
							.toString() + " ");

				// print objectives
				pw.println(aSolutionsList_.toString());
			}
		}
	}
} // SolutionSet
