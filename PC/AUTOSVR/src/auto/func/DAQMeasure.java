...
public class DAQMeasure implements SerialPortEventListener {
	...
	public Double measure(Profile pro) throws Exception {
		// power on the sensor
		String[] exec_cmd = { "powertoolcmd", "/vout=3.5",
				"/trigger=ETD" + String.valueOf(4 * _mtime) + "A",
				"/savefile=datafile.pt4", "/noexitwait" };
		Process p = Runtime.getRuntime().exec(exec_cmd);
		FileUtils.logFile.println("[MONITOR] Power on sensor ("
...
//		Thread.sleep(1000);
		...
		FileUtils.logFile.println("[MONITOR] PC_CMD_ACT msg sent: " + (2*_mtime) +  " ("
				+ new java.util.Date().toString().substring(11, 19) + ")");
		_finish = false;
		while (!_finish) {
		}
//		Thread.sleep(1000);
		...
		p.destroy();
		FileUtils.logFile.println("[MONITOR] Power off sensor with exit code " 
				+ ret + " (" 
				+ new java.util.Date().toString().substring(11, 19) + ")");
		FileUtils.logFile.flush();

		// collect the measurement
		FileUtils.logFile.println("[MONITOR] Collecting power measurement (" 
				+ new java.util.Date().toString().substring(11, 19) + ")");
		FileUtils.logFile.flush();
		if (_result != null) {
				...
				while (inputStream.available() > 0/* && !bflag*/) {
					...
					if (pre == 0) { // data info
						...
						case 1: // SNR_CMD_RDY
							...
							FileUtils.logFile.println("[MONITOR] SNR_CMD_RDY msg received ("
				+ new java.util.Date().toString().substring(11, 19) + ")");
							fname = mycmd.send_to_sensor_pin_config_1(_pro);
							FileUtils.logFile.println("[MONITOR] PC_CMD_CFG msg sent: ("
				+ new java.util.Date().toString().substring(11, 19) + ")");
						...
						case 3: // SNR_CMD_LPW_BGN
							...
							FileUtils.logFile.println("[MONITOR] SNR_CMD_LPW_BGN msg received ("
				+ new java.util.Date().toString().substring(11, 19) + ")");
							if (fname.length() != 0) {
								...
								FileUtils.logFile.println("[MONITOR] begin to run IviDemo ("
				+ new java.util.Date().toString().substring(11, 19) + ")");
								Process p = Runtime.getRuntime().exec(exec_cmd);
								int ret = p.waitFor();
								...
								p.destroy();
								FileUtils.logFile.println("[MONITOR] end running IviDemo ("
				+ new java.util.Date().toString().substring(11, 19) + ")");
								FileUtils.logFile.flush();
								bflag = true;
							}
						...
						case 4: // SNR_CMD_LPW_END
							// send_to_monitor_end();
							FileUtils.logFile.println("[MONITOR] SNR_CMD_LPW_END msg received ("
				+ new java.util.Date().toString().substring(11, 19) + ")");
							FileUtils.logFile.flush();
							bflag = true;
							break;
